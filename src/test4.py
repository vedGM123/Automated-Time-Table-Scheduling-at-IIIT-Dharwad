# TT_gen_fixed.py -- Complete Fixed Timetable Generator with Multi-Section Support
# Key improvements:
# 1. Proper handling of CSE 2-section split
# 2. C004 room allocation for large combined classes
# 3. Per-section room assignments
# 4. Better capacity checking
...
# Meta entries collected while writing timetables
META_ENTRIES = []
...
# Ensure 'total_students' column exists
...
print(f"✅ Processed student strengths (defaulting to 50 if missing)")
...
# Create global lookup dictionary
ROOM_DATA = {}
...
def generate_time_slots():
    """Generate time slots with no morning break and lunch 13:00-14:00"""
    slots = []
    ...
    slots.append((time(18, 30), time(20, 0)))  # 90 min
...
def is_break_time_slot(slot, semester=None):
    """Check if slot is lunch break (13:00-14:00)"""
    start, end = slot
...
def is_c004_required(course_code):
    """Check if course code explicitly requires C004 room"""
    return '-C004' in str(course_code).upper()
...
def find_suitable_room_for_slot(course_code, room_type, day, slot_indices, room_schedule,
    ...
    """
    Find suitable room with proper type and capacity checking.
    Handles C004 forced assignments and multi-section support.
    """
    mapping_key = f"{course_code}_{component_type}"
    
    # Check if C004 is explicitly required
    if is_c004_required(course_code):
        forced_room = "C004"
    ...
    # Special handling for large courses (>120 students)
    ...
    # Try to combine lab rooms for labs
    if room_type == 'COMPUTER_LAB':
...
def generate_all_timetables():
    ...
    for department in df['Department'].unique():
        ...
        for semester in sems:
            ...
            dept_upper = str(department).strip().upper()
            # CSE gets 2 sections, others get 1
            ...
            for section in range(num_sections):
                ...
                section_suffix = '' if num_sections == 1 else f"_{chr(65 + section)}"
                ...
                for _, course in elective_courses_in_section.iterrows():
                    ...
                    for day, slot_indices, comp_type in basket_schedule:
                        ...
                        total_students_raw = int(course.get('total_students', 50))
                        
                        # For C004 courses, use full student strength
                        if is_c004_required(code):
                            student_strength = total_students_raw
                        else:
                            student_strength = max(1, total_students_raw // max(1, num_sections))
                        
                        code_with_section = f"{code}{section_suffix}"
                        candidate_room = find_suitable_room_for_slot(
                            code_with_section, room_type, day, slot_indices,
                        ...
                        for si_idx, si in enumerate(slot_indices):
                            if timetable[day][si]['type'] is None:
                                ...
                                course_room_mapping[f"{code_with_section}_{comp_type}"] = candidate_room
                ...
                for _, course in courses_combined.iterrows():
                    ...
                    base_code = get_base_course_code(code)
                    base_code_with_section = f"{base_code}{section_suffix}"
                    name = str(course.get('Course Name', '')).strip()
                    faculty = select_faculty(course.get('Faculty', 'TBD'))
                    total_students_raw = int(course.get('total_students', 50))
                        ...
                        for attempt in range(attempts_limit):
                            ...
                            for start_idx in starts:
                                slot_indices, candidate_room = find_consecutive_slots_for_minutes(
                                    ...
                                    base_code_with_section, course_room_mapping, comp_type, course_day_components,
                                    student_strength
                                )
                                ...
                                for si_idx, si in enumerate(slot_indices):
                                    ...
                                    if candidate_room not in room_schedule:
                                        room_schedule[candidate_room] = {d: set() for d in range(len(DAYS))}
                                    room_schedule[candidate_room][day].add(si)
                                course_room_mapping[f"{base_code_with_section}_{comp_type}"] = candidate_room
    ...
    for day_idx, day_name in enumerate(DAYS):
        ...
        for slot_idx in range(len(TIME_SLOTS)):
            ...
            if code:
                ...
                basket_label = None
                base_code = ''
                if parts and re.match(r'^B\d+', parts[0].upper()):  # FIXED
                    basket_label = parts[0].upper()
                    if len(parts) > 1:
                        base_code = parts[1]
                elif re.match(r'^B\d+', raw_code.strip().upper()):  # FIXED
...
if __name__ == "__main__":
    try:
        print("\n" + "="*80)
        print("🎓 IIIT DHARWAD TIMETABLE GENERATOR - MULTI-SECTION SUPPORT")
        print("="*80)
        print("\n🔧 Configuration:")
        ...
        print(f"    - CSE department: 2 sections (A & B)")
        print(f"    - C004 room: Auto-assigned to courses with -C004 suffix")
        print("="*80 + "\n")
        ...
        print("       • CSE_1_A and CSE_1_B (separate sections)")
        print("       • CSE_3_A and CSE_3_B (separate sections)")
        print("       • CSE_5_A and CSE_5_B (separate sections)")
        print("       • DSAI and ECE (single sections)")
        print("       • Common_7th_Semester (combined CSE/DSAI/ECE)")
        ...
        print("\n💡 Key Features:")
        print("    - Courses with -C004 suffix use the 240-seater hall")
        print("    - Non-C004 courses split across sections get separate rooms")
        print("    - Room capacity matched to student strength per section")
        print("    - Lab courses can combine multiple rooms if needed")
        print("\n⚠️ Important Notes:")
        print("    - Check unscheduled_courses.xlsx for any failed schedules")
        print("    - Large elective baskets (>120 students) auto-assigned to C004")
        print("    - Each CSE section gets ~107-108 students per course")
        print("="*80 + "\n")
